"""CLI entry point for the Resource Scout agent."""
...
@click.group()
def cli():
    """Ideation Agent: Resource Scout - Technical feasibility & resources."""
    pass
...
def run(session_id: str, problem: str | None, output_json: bool):
    ...
    console.print(f"[blue]Running resource-scout agent for session: {session_id}[/blue]")
    result = run_agent(session_id, problem)
    memory.update_phase("resource_scout", "complete", result)

    if output_json:
        print(json.dumps(result, indent=2))
    else:
        console.print("[green]Resource Scout agent completed successfully[/green]")
